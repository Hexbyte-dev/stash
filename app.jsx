...
// ============================================================
const TYPE_COLORS = {
  note: "#8B9E6B",      // sage green
  link: "#5B8FA8",      // ocean blue
  contact: "#C4956A",   // warm copper
  reading: "#7B6B8A",   // muted purple
  food: "#D4915E",      // terracotta
  idea: "#C9A84C",      // golden
  person: "#C4956A",    // warm copper (same as contact)
  photo: "#5BA88F",     // teal green
  travel: "#5B8FA8",    // ocean blue (same as link)
  work: "#8B7355",      // brown
...
const exportData = (items, settings) => {
  const exportObj = {
    ...
    version: APP_VERSION, // Fix #17: references shared constant
...
const SettingsPanel = ({ isOpen, onClose, settings, onUpdateSettings, theme, itemCount, completedCount, onExport, onExportFull, onImport, onLogout, onSaveDigestSettings }) => {
    ...
    >
      ...
      >
        ...
        <div style={{
          ...
          onMouseEnter={e => { e.currentTarget.style.color = theme.textMuted; e.currentTarget.style.background = theme.hoverBg; }}
          onMouseLeave={e => { e.currentTarget.style.color = theme.textGhost; e.currentTarget.style.background = "none"; }}
          >×</button>
        </div>
                ...
                return (
                  <div key={cat.id} style={{
                    ...
                    <button
                      ...
                      onMouseEnter={e => { e.currentTarget.style.color = theme.deleteColor; }}
        ...
        <div style={{
          ...
          {/* Frequency radio buttons — "radio" inputs let the user pick
              ...
              allow multiple). All radios with the same "name" attribute
              are automatically grouped together by the browser. */}
              ...
              <label key={opt.value} style={{
                ...
                <input
                  ...
                  onChange={() => {
                    ...
                    onUpdateSettings({ ...settings, digestFrequency: opt.value });
                    // Also save to server (for the digest worker to read)
                    onSaveDigestSettings({ digest_frequency: opt.value });
                  }}
          ...
          {/* Send time & timezone dropdowns — only show when digest is enabled.
              This is "conditional rendering": the && operator means
              "if the left side is true, render the right side." */}
          {settings.digestFrequency !== "none" && (
            <>
              <div style={{ marginBottom: "16px" }}>
                <div style={{
                  fontFamily: "'DM Sans', sans-serif", fontSize: "12.5px",
                ...
                <select
                  ...
                  onChange={(e) => {
                    onUpdateSettings({ ...settings, digestTime: e.target.value });
                    onSaveDigestSettings({ digest_time: e.target.value });
                  }}
                  style={{
                    background: theme.hoverBg,
                    border: `1px solid ${theme.border}`,
                ...
                >
                  {DIGEST_TIMES.map(t => (
                    <option key={t} value={t}>
                      {/* Convert 24h to 12h for friendlier display: "08:00" → "8:00 AM" */}
                      {parseInt(t) === 0 ? "12:00 AM"
                        : parseInt(t) < 12 ? `${parseInt(t)}:00 AM`
                        : parseInt(t) === 12 ? "12:00 PM"
                        : `${parseInt(t) - 12}:00 PM`}
                    </option>
                  ))}
                </select>
              ...
              <div style={{ marginBottom: "4px" }}>
                ...
                <select
                  ...
                  onChange={(e) => {
                    onUpdateSettings({ ...settings, digestTimezone: e.target.value });
                    onSaveDigestSettings({ timezone: e.target.value });
                  }}
                  style={{
                    background: theme.hoverBg,
                    border: `1px solid ${theme.border}`,
        ...
        <div style={{
          ...
          <div style={{
            fontFamily: "'Lora', serif", fontSize: "12.5px",
            color: theme.textFaint, fontStyle: "italic", marginBottom: "14px",
          ...
          <form onSubmit={handleChangePassword}>
            <input
              ...
              style={{
                ...
                color: theme.textPrimary, fontFamily: "'DM Sans', sans-serif",
            ...
            {pwError && (
              <div style={{
                color: theme.deleteColor, fontSize: "12.5px",
                fontFamily: "'DM Sans', sans-serif", marginBottom: "8px",
              }}>{pwError}</div>
            )}
            {pwSuccess && (
              <div style={{
                color: theme.checkColor, fontSize: "12.5px",
                fontFamily: "'DM Sans', sans-serif", marginBottom: "8px",
              }}>{pwSuccess}</div>
            )}
            <button
        ...
        <div style={{
          ...
          <div style={{
            fontFamily: "'DM Sans', sans-serif", fontSize: "14.5px",
            color: theme.deleteColor, fontWeight: 500, marginBottom: "3px",
          }}>Danger zone</div>
          <div style={{
            fontFamily: "'Lora', serif", fontSize: "12.5px",
            color: theme.textFaint, fontStyle: "italic", marginBottom: "12px",
          }}>Irreversible actions</div>

          {!showDeleteConfirm ? (
            <button
              onClick={() => setShowDeleteConfirm(true)}
              style={{
                background: "none", border: `1px solid ${theme.deleteColor}`,
                color: theme.deleteColor, borderRadius: "10px",
                padding: "8px 16px", fontSize: "13px",
                fontFamily: "'DM Sans', sans-serif",
            ...
            <div>
              ...
              <input
                ...
                style={{
                  width: "100%", padding: "10px 12px", borderRadius: "10px",
                  border: `1px solid ${theme.deleteColor}`, background: theme.inputBg,
                  color: theme.textPrimary, fontFamily: "'DM Sans', sans-serif",
                  fontSize: "13px", outline: "none", boxSizing: "border-box",
                  marginBottom: "10px",
                }}
              ...
              <div style={{ display: "flex", gap: "8px" }}>
                <button
                  ...
                  style={{
                    ...
                    color: theme.textSecondary, fontFamily: "'DM Sans', sans-serif",
...
// ============================================================
const ReminderPicker = ({ onPick, onClose, theme }) => {
  ...
  return (
    ...
    <div onClick={onClose} style={{
      ...
      backdropFilter: "blur(8px)", zIndex: 950,
      display: "flex", alignItems: "center", justifyContent: "center",
      animation: "softFadeIn 0.2s ease",
    }}>
      {/* Inner card — stopPropagation prevents clicks inside from closing */}
      <div onClick={e => e.stopPropagation()} style={{
        background: theme.settingsBg, borderRadius: "20px",
        border: `1px solid ${theme.border}`,
        padding: "28px 24px", width: "92%", maxWidth: "340px",
        boxShadow: theme.shadowHover,
        animation: "settingsIn 0.3s ease",
      }}>
        <h3 style={{
          fontFamily: "'Lora', serif", fontSize: "18px", fontWeight: 400,
          color: theme.textSecondary, margin: "0 0 18px",
        }}>Set Reminder</h3>

        {/* Quick presets — stacked vertically for easy tapping on mobile */}
        ...
        {/* Custom date/time — for when presets don't fit.
            Uses native HTML date/time inputs which show platform pickers. */}
        <div style={{
          borderTop: `1px solid ${theme.border}`, paddingTop: "16px",
        }}>
          <div style={{
            fontFamily: "'DM Sans', sans-serif", fontSize: "12px",
            color: theme.textMuted, marginBottom: "8px", fontWeight: 500,
          }}>Custom time</div>
          <div style={{ display: "flex", gap: "8px", marginBottom: "10px" }}>
            <input type="date" value={customDate} onChange={e => setCustomDate(e.target.value)}
              style={{
                flex: 1, background: theme.hoverBg, border: `1px solid ${theme.border}`,
                borderRadius: "8px", padding: "7px 10px", color: theme.textPrimary,
                fontSize: "12px", fontFamily: "'DM Sans', sans-serif",
              }}
            />
            <input type="time" value={customTime} onChange={e => setCustomTime(e.target.value)}
              style={{
                width: "110px", background: theme.hoverBg, border: `1px solid ${theme.border}`,
                borderRadius: "8px", padding: "7px 10px", color: theme.textPrimary,
                fontSize: "12px", fontFamily: "'DM Sans', sans-serif",
              }}
            />
        ...
        <button onClick={onClose} style={{
          ...
          color: theme.textGhost, fontSize: "13px",
          fontFamily: "'DM Sans', sans-serif", cursor: "pointer",
        }}>Cancel</button>
      </div>
    </div>
...
// ============================================================
const StashCard = ({ item, onDelete, onToggleComplete, onEdit, onViewImage, onTogglePin, onSetReminder, onScanCard, isScanning, theme, timezone, customCategories, isSlidingOut, bulkMode, isSelected, onToggleBulkSelect, searchQuery }) => {
  ...
  return (
    <div
      ...
      borderLeft: item.pinned && !isSelected
        ? `4px solid ${theme.checkColor}66`
        : `4px solid ${TYPE_COLORS[item.type] || '#C4B5A5'}`,
    }}
    onMouseEnter={e => {
      ...
      <div style={{ display: "flex", gap: "14px", alignItems: "flex-start" }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "8px" }}>
            {/* Type pill — uses the TYPE_COLORS map for a tinted background (15% opacity)
                so each type has a distinctive but subtle color identity on the card */}
            <span style={{
              fontSize: "11px",
            ...
            {/* Reminder indicator — shows when a reminder is set on this card.
                Changes color to red (deleteColor) if the reminder is overdue. */}
            {item.reminder && !isCompleted && (
              <span style={{
                ...
                fontFamily: "'DM Sans', sans-serif",
                fontStyle: "italic",
              }}>&#x23F0; {new Date(item.reminder).toLocaleDateString("en-US", { month: "short", day: "numeric" })}</span>
            )}
          </div>
          ...
          {isEditing ? (
            ...
            <div>
              {/* LINK-TYPE: Make URLs clickable so users can visit saved links directly.
                  We detect URLs using a regex and wrap them in <a> tags.
                  target="_blank" opens in a new tab; rel="noopener noreferrer" is a security
                  best practice that prevents the new page from accessing our window object. */}
              {item.type === "link" && item.content && !isEditing && (() => {
                ...
                return (
                  <p onClick={startEditing} style={{
                    margin: 0, color: theme.textPrimary, fontSize: "15.5px",
                    ...
                    {parts.map((part, i) => {
                      ...
                      return urlRegex.test(part)
                        ? <a key={i} href={part} target="_blank" rel="noopener noreferrer"
                            ...
                            }}><HighlightText text={part} query={searchQuery} theme={theme} /></a>
                        : <HighlightText key={i} text={part} query={searchQuery} theme={theme} />;
                    })}
                  </p>
                );
              })()}

              {/* CONTACT-TYPE: Show structured fields from OCR data instead of raw text.
                  When we scan a business card, Claude extracts fields like name, phone, email
                  into ocrData. Here we display those fields with icons, making contacts
                  scannable at a glance instead of showing a wall of unformatted text.
                  Works for both "contact" and "person" type stashes. */}
              {(item.type === "contact" || item.type === "person") && item.ocrData && !isEditing && (
                <div style={{ marginTop: "4px" }}>
                  {/* Name gets the biggest, most prominent styling */}
                  {item.ocrData.name && (
                    <div style={{ fontSize: "17px", fontFamily: "'Lora', serif", fontWeight: 500, color: theme.textPrimary, marginBottom: "6px" }}>
                      <HighlightText text={item.ocrData.name} query={searchQuery} theme={theme} />
                    </div>
                  )}
                  {/* Title + company on one line when both exist, e.g. "Designer at Acme" */}
                  {item.ocrData.title && (
                    <div style={{ fontSize: "13.5px", fontFamily: "'DM Sans', sans-serif", color: theme.textSecondary, marginBottom: "2px" }}>
                      <HighlightText text={item.ocrData.title} query={searchQuery} theme={theme} />
                      {item.ocrData.company && <span> at <HighlightText text={item.ocrData.company} query={searchQuery} theme={theme} /></span>}
                    </div>
                  )}
                  {/* Show company alone only if there's no title (avoids duplication) */}
                  {!item.ocrData.title && item.ocrData.company && (
                    <div style={{ fontSize: "13.5px", fontFamily: "'DM Sans', sans-serif", color: theme.textSecondary, marginBottom: "2px" }}>
                      <HighlightText text={item.ocrData.company} query={searchQuery} theme={theme} />
                    </div>
                  )}
                  {/* Contact detail rows — each is a clickable action (call, email, visit).
                      We use <a> tags with tel:, mailto:, and https:// protocols so the
                      phone/browser knows how to handle each type of link natively. */}
                  <div style={{ display: "flex", flexDirection: "column", gap: "4px", marginTop: "8px" }}>
                    {item.ocrData.phone && (
                      <a href={`tel:${item.ocrData.phone.replace(/[^\d+\-() ]/g, "")}`} onClick={e => e.stopPropagation()} style={{
                        fontSize: "13px", fontFamily: "'DM Sans', sans-serif",
                    ...
                    {item.ocrData.email && (
                      <a href={`mailto:${item.ocrData.email.split("?")[0]}`} onClick={e => e.stopPropagation()} style={{
                        fontSize: "13px", fontFamily: "'DM Sans', sans-serif",
                        color: theme.textMuted, textDecoration: "none",
                        display: "flex", alignItems: "center", gap: "8px",
                      }}>
                    ...
                    {item.ocrData.website && (
                      <a href={item.ocrData.website.startsWith("http") ? item.ocrData.website : `https://${item.ocrData.website}`}
                        target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} style={{
                        fontSize: "13px", fontFamily: "'DM Sans', sans-serif",
                        color: theme.textMuted, textDecoration: "none",
                        display: "flex", alignItems: "center", gap: "8px",
                      }}>
                    ...
                    {item.ocrData.address && (
                      <div style={{
                        fontSize: "13px", fontFamily: "'DM Sans', sans-serif",
                        color: theme.textGhost, display: "flex", alignItems: "center", gap: "8px",
                        marginTop: "2px",
                      }}>
                        <span style={{ fontSize: "11px", opacity: 0.6 }}>◇</span>
                        {item.ocrData.address}
              ...
              {/* DEFAULT LAYOUT: Used for all other types (note, idea, food, etc.)
                  and also for contacts/persons that don't have ocrData yet.
                  The condition excludes link-type (handled above) and contact/person
                  with ocrData (also handled above) so we don't render content twice. */}
              {item.content && !(item.type === "link" && !isEditing) && !((item.type === "contact" || item.type === "person") && item.ocrData && !isEditing) && (
                <p
                  ...
                  onMouseLeave={e => { e.currentTarget.style.background = "transparent"; }}
                  title={isCompleted ? "" : "Click to edit"}
                ><HighlightText text={item.content} query={searchQuery} theme={theme} /></p>
              )}
                ...
                >
                  {item.tags.map(tag => (
                    <span key={tag} style={{
                      ...
                      borderRadius: "8px",
                      fontFamily: "'DM Sans', sans-serif",
                    }}>#<HighlightText text={tag} query={searchQuery} theme={theme} /></span>
                  ))}
                </div>
              )}
        ...
        {!isEditing && (
          <div style={{
            ...
            <button
              ...
              onMouseEnter={e => { if (!item.pinned) { e.currentTarget.style.color = theme.textMuted; e.currentTarget.style.background = theme.hoverBg; } }}
              onMouseLeave={e => { if (!item.pinned) { e.currentTarget.style.color = theme.textGhost; e.currentTarget.style.background = "none"; } }}
              title={item.pinned ? "Unpin" : "Pin to top"}
            >{item.pinned ? "Pinned" : "Pin"}</button>
            {/* Reminder button — lets users set a future time to be notified about this stash.
                Uses the browser's Notification API to send a desktop/mobile alert.
                If a reminder is already set, clicking clears it (toggle behavior). */}
            <button
              onClick={(e) => {
                e.stopPropagation();
              ...
              onMouseEnter={e => { if (!item.reminder) { e.currentTarget.style.color = theme.textMuted; e.currentTarget.style.background = theme.hoverBg; } }}
            ...
            {displayImage && !item.ocrData && (
              <button
                ...
                onMouseEnter={e => { if (!isScanning) { e.currentTarget.style.color = theme.textMuted; e.currentTarget.style.background = theme.hoverBg; } }}
                onMouseLeave={e => { if (!isScanning) { e.currentTarget.style.color = theme.textGhost; e.currentTarget.style.background = "none"; } }}
            ...
            <button
              ...
              onMouseEnter={e => { e.currentTarget.style.color = theme.deleteColor; e.currentTarget.style.background = theme.deleteBg; }}
              onMouseLeave={e => { e.currentTarget.style.color = theme.textGhost; e.currentTarget.style.background = "none"; }}
              title="Remove"
              aria-label="Close"
            >×</button>
...
// ============================================================
const AnalyticsDashboard = ({ isOpen, onClose, items, theme, customCategories }) => {
  ...
  const avgPerDay = totalItems > 0
    ? (totalItems / Math.max(1, Math.ceil((now - new Date(items[items.length - 1]?.createdAt || now)) / (1000 * 60 * 60 * 24)))).toFixed(1)
    : "0";
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Summary cards row — three key numbers at a glance */}
        <div style={{
          display: "grid", gridTemplateColumns: "1fr 1fr 1fr",
          gap: "12px", marginBottom: "28px",
        }}>
          {[
            { value: totalItems, label: "Total", color: theme.textSecondary },
            { value: completedCount, label: "Done", color: theme.checkColor },
            { value: `${streak}d`, label: "Streak", color: theme.accent },
          ].map(stat => (
            <div key={stat.label} style={{
              textAlign: "center", padding: "14px 8px",
              background: theme.hoverBg, borderRadius: "14px",
              border: `1px solid ${theme.border}`,
            }}>
              <div style={{
                fontFamily: "'Lora', serif", fontSize: "26px",
        ...
        {/* Mini stats row — secondary metrics in a compact layout */}
        <div style={{
          ...
          {[
            ...
            { value: withImages, label: "photos" },
            { value: avgPerDay, label: "per day" },
            { value: `${completionRate}%`, label: "done" },
          ].map(s => (
        ...
        {/* Activity chart (last 30 days)
            CSS-only bar chart: each bar's height is proportional to that
            day's count divided by the max daily count. No chart library needed! */}
        <div style={{ marginBottom: "24px" }}>
          <div style={{
            fontFamily: "'DM Sans', sans-serif", fontSize: "13px",
            color: theme.textMuted, fontWeight: 500, marginBottom: "10px",
          }}>Last 30 days</div>
          <div style={{
            display: "flex", alignItems: "flex-end", gap: "2px",
            height: "60px", padding: "0 2px",
          }}>
            {activityValues.map((count, i) => (
              <div key={i} style={{
                flex: 1, minWidth: "4px",
        ...
        {/* By type (horizontal bar chart)
            Each bar width = (count / maxTypeCount) * 100%. The type with
            the most items always fills the full width; others are proportional. */}
              ...
              {sortedTypes.map(([type, count]) => {
                ...
                return (
                  <div key={type} style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                    <span style={{
                      fontFamily: "'DM Sans', sans-serif", fontSize: "12px",
                      color: theme.textMuted, width: "80px", textAlign: "right",
                    }}>{info.label}</span>
                    <div style={{
                      flex: 1, height: "18px", background: theme.hoverBg,
                    ...
                    <span style={{
                      fontFamily: "'DM Sans', sans-serif", fontSize: "12px",
                      color: theme.textGhost, width: "30px",
                    }}>{count}</span>
                  </div>
                );
              })}
        ...
        {/* Top tags — displayed as pill/chip badges */}
        {topTags.length > 0 && (
          <div>
            <div style={{
              fontFamily: "'DM Sans', sans-serif", fontSize: "13px",
            ...
            <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
              {topTags.map(([tag, count]) => (
                <span key={tag} style={{
                  ...
                  background: theme.hoverBg, padding: "4px 12px",
                  borderRadius: "20px", fontFamily: "'DM Sans', sans-serif",
                  border: `1px solid ${theme.border}`,
                }}>#{tag} <span style={{ color: theme.textGhost, fontSize: "11px" }}>{count}</span></span>
              ))}
            </div>
          </div>
        )}
  ...
  const handleForgotReset = async (e) => {
    ...
    try {
      const res = await fetch(`${BACKEND_URL}/api/auth/reset-password`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include", // Receive the httpOnly cookie from the server
  ...
  if (forgotStep) {
    return (
      <div style={{
        ...
        display: "flex", alignItems: "center", justifyContent: "center",
        fontFamily: "'DM Sans', sans-serif", transition: "background 0.4s ease",
      }}>
        <div style={{ width: "92%", maxWidth: "380px", animation: "softFadeIn 0.6s ease" }}>
            ...
            {forgotStep === 1 && (
              <form onSubmit={handleForgotSubmitEmail}>
                <p style={{
                  ...
                  color: theme.textSecondary, marginBottom: "18px", lineHeight: 1.5,
                ...
                {forgotError && (
                  <div style={{
                    color: theme.deleteColor, fontSize: "13px",
                    fontFamily: "'DM Sans', sans-serif", marginBottom: "14px", textAlign: "center",
                  }}>{forgotError}</div>
                )}
                <button
                  type="submit"
              ...
              <form onSubmit={handleForgotReset}>
                ...
                <div style={{ marginBottom: "14px" }}>
                  <label style={{
                    ...
                    fontSize: "13px", fontWeight: 500, color: theme.textMuted, marginBottom: "6px",
                  }}>New password</label>
                  <input
                    type="password"
        ...
        <div style={{
          ...
          {authMode === "login" && (
            <div style={{
              ...
              <span
                onClick={() => { setForgotStep(1); setForgotEmail(email); setError(""); }}
                style={{ color: theme.textFaint, cursor: "pointer" }}
...
function Stash() {
  ...
  useEffect(() => {
    const handleGlobalKeyDown = (e) => {
      ...
      if (e.key === "Escape") {
        if (reminderPickerId)   { setReminderPickerId(null); return; }
        if (viewingImage)       { setViewingImage(null);  return; }
  ...
  useEffect(() => {
    if (settings.autoArchiveDays > 0 && !isLoading && token) {
      ...
      const toArchive = currentItems.filter(item =>
        item.completed && item.completedAt &&
        (now - new Date(item.completedAt).getTime()) >= threshold
      );
      if (toArchive.length > 0) {
        setItems(prev => prev.filter(i => !toArchive.some(a => a.id === i.id)));
        // Sync to server
        toArchive.forEach(item => {
          apiFetch(`/api/stashes/${item.id}`, { method: "DELETE" })
  ...
  const bulkRetype = (ids, newType) => {
    ...
    setItems(prev => prev.map(item =>
      ids.has(item.id) ? { ...item, type: newType } : item
    ));
    // Sync each to server
    ids.forEach(id => {
      apiFetch(`/api/stashes/${id}`, {
  ...
  const setReminder = (id, value) => {
    ...
    setItems(prev => prev.map(item =>
      item.id === id ? { ...item, reminder: value } : item
    ));
  ...
  useEffect(() => {
    const checkReminders = () => {
      ...
      if (firedIds.size > 0) {
        setItems(prev => prev.map(i =>
          firedIds.has(i.id) ? { ...i, reminder: null } : i
        ));
      }
    };
  ...
  const filteredItems = items.filter(item => {
    ...
    const matchesTags = filterTags.length === 0 ||
      filterTags.every(ft => (item.tags || []).some(t => t.toLowerCase().includes(ft)));
  ...
  return (
    ...
    >
        ...
        onImport={(data) => {
          if (data?.items && Array.isArray(data.items)) {
            ...
            const validItem = (item) =>
              ...
              (!item.tags || (Array.isArray(item.tags) && item.tags.every(t => typeof t === 'string'))) &&
              (!item.image || (typeof item.image === 'string' && item.image.length <= 7 * 1024 * 1024));
            ...
            if (data.settings) {
              ...
              const safeSettings = {};
              allowedKeys.forEach(key => { if (data.settings[key] !== undefined) safeSettings[key] = data.settings[key]; });
            ...
            apiFetch("/api/stashes/import", {
              method: "POST",
              body: JSON.stringify({ stashes: validItems }),
            }).then(() => {
      ...
      {/* Email verification modal — appears when user clicks "Send Code" on the banner */}
      {showVerifyModal && (
        <div
          onClick={() => { setShowVerifyModal(false); setVerifyError(""); setVerifyCode(""); }}
          style={{
            position: "fixed", inset: 0,
            background: theme.overlayBg,
            backdropFilter: "blur(16px)",
        ...
        >
          ...
          >
            <h3 style={{
              fontFamily: "'Lora', serif", fontSize: "20px", fontWeight: 400,
              color: theme.textSecondary, margin: "0 0 8px",
            }}>Verify your email</h3>
            <p style={{
              fontFamily: "'DM Sans', sans-serif", fontSize: "13px",
              color: theme.textFaint, margin: "0 0 20px",
            }}>Enter the 6-digit code sent to your email</p>
            <input
              type="text"
              value={verifyCode}
            ...
            {verifyError && (
              <div style={{
                color: theme.deleteColor, fontSize: "13px",
                fontFamily: "'DM Sans', sans-serif", marginBottom: "12px", textAlign: "center",
              }}>{verifyError}</div>
            )}
            <div style={{ display: "flex", gap: "10px" }}>
              ...
              <button
                ...
                style={{
                  ...
                  fontFamily: "'DM Sans', sans-serif",
                  fontSize: "14px", fontWeight: 500, cursor: (verifyCode.length !== 6 || verifyLoading) ? "default" : "pointer",
                }}
              >{verifyLoading ? "Verifying..." : "Verify"}</button>
            </div>
          </div>
        </div>
      )}

      {/* ===== MAIN CONTENT — 720px max gives cards more breathing room on desktop ===== */}
      <div style={{
        maxWidth: "720px", margin: "0 auto", padding: "40px 20px",
        animation: "softFadeIn 0.6s ease",
      }}>
        {/* Email verification banner — shows when user hasn't verified their email */}
        {!emailVerified && token && (
          <div style={{
            background: "#FFF3CD",
            ...
            <span style={{
              ...
              fontSize: "13.5px",
              color: "#5C5347",
            }}>Please verify your email to enable digest emails.</span>
            <button
            ...
            >{verifyLoading ? "Sending..." : "Send Code"}</button>
          </div>
        )}
        {verifyError && !showVerifyModal && (
          <div style={{
            color: theme.deleteColor, fontSize: "13px",
            fontFamily: "'DM Sans', sans-serif", marginBottom: "12px", textAlign: "center",
          }}>{verifyError}</div>
        ...
        {/* Header — slightly larger title with letter-spacing for a "branded" feel,
            plus a subtle bottom border gradient to separate header from content */}
        <header style={{
          marginBottom: "36px",
              ...
              {getAllTypes(settings.customCategories).map(type => {
                ...
                return (
                  <button
                    ...
                    onMouseEnter={e => { if (!isActive) e.currentTarget.style.color = typeColor; }}
        ...
        {items.length > 0 && (
          <div style={{ marginBottom: "20px" }}>
            <div style={{
              ...
              {/* Keyboard shortcut hint — a small muted pill like GitHub/Slack
                  shows so power users discover the shortcut. Hidden when
                  there's an active query to avoid visual clutter. */}
              {!searchQuery && (
                <span style={{
                  fontSize: "10px", fontFamily: "'DM Sans', sans-serif",
                  color: theme.textGhost, background: theme.border + "44",
                  border: `1px solid ${theme.border}`,
                  borderRadius: "4px", padding: "2px 6px",
                  whiteSpace: "nowrap", userSelect: "none",
                  lineHeight: "1.4",
                }}>
                  {/* navigator.platform is deprecated but still the simplest
                      way to detect Mac for cosmetic UI hints */}
                  {navigator.platform?.includes("Mac") ? "⌘K" : "Ctrl+K"}
                </span>
              )}
            </div>
            {/* Filter bar — rounded pills with subtle borders when inactive,
                and colored backgrounds when active. Makes the filters feel clickable. */}
            <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
              <button onClick={() => setFilterType("all")} aria-pressed={filterType === "all"} style={{
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: "8px" }}>
              {/* ============================================================
                  SELECT / SELECT ALL / NONE BUTTONS

                  The main "Select" button toggles bulk mode on/off.
                  When bulk mode is active, "All" and "None" appear beside it:
                  - "All" adds every currently VISIBLE (filtered) item to the
                    selection. This is useful when you want to act on most items
                    — select all, then deselect a few.
                  - "None" clears the selection without leaving bulk mode.
                  ============================================================ */}
              <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                <button
                  onClick={() => { setBulkMode(prev => !prev); setBulkSelected(new Set()); setBulkRetypeOpen(false); setBulkTagInput(""); }}
                  style={{
                ...
                {/* Select All / None — only visible when bulk mode is active.
                    "Select All" adds every visible (filtered) item ID to the set.
                    "None" clears the set. This saves time when you want to
                    act on most items — select all, then deselect a few. */}
                {bulkMode && (
                  <>
                    <button
                      onClick={() => {
            ...
            <div style={{ marginTop: "8px" }}>
              ...
              >
                {/* The arrow character rotates 90° when the panel opens.
                    transform + transition create a smooth spin effect. */}
                <span style={{
                ...
                Advanced filters
                {/* "active" pill — only rendered when a filter is set */}
                {(dateFrom || dateTo || tagFilter) && (
                  <span style={{
                    fontSize: "10px", background: theme.accent + "22",
                    color: theme.accent, borderRadius: "10px",
                    padding: "1px 8px",
                  }}>active</span>
                )}
              </button>

              {/* The panel itself — only mounts when advancedOpen is true.
                  This is called "conditional rendering": React skips
                  everything inside the `&&` when the left side is false. */}
              {advancedOpen && (
                <div style={{
                  ...
                  <div style={{ marginBottom: "14px" }}>
                    ...
                    <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
                      {/* <input type="date"> renders a native date picker.
                          Its value is an ISO string like "2026-03-05". */}
                      <input
                        type="date" value={dateFrom}
                        onChange={e => setDateFrom(e.target.value)}
                        style={{
                          flex: 1, background: theme.hoverBg,
                          border: `1px solid ${theme.border}`,
                      ...
                      />
                      <span style={{ color: theme.textGhost, fontSize: "12px" }}>to</span>
                      <input
                  ...
                  <div style={{ marginBottom: "14px" }}>
                    ...
                    {/* A plain text input — the filtering logic in
                        filteredItems splits on commas and matches. */}
                    <input
                      type="text" value={tagFilter}
                      onChange={e => setTagFilter(e.target.value)}
                      placeholder="e.g. recipe, italian"
                      style={{
                        width: "100%", background: theme.hoverBg,
                        border: `1px solid ${theme.border}`,
                    ...
                    <div style={{
                      fontFamily: "'DM Sans', sans-serif", fontSize: "11px",
                      color: theme.textGhost, fontStyle: "italic", marginTop: "4px",
                    }}>Comma-separated — shows items with ALL specified tags</div>
                  </div>

                  {/* ── Clear Filters ───────────────────────────────── */}
                  {/* Only show the button when at least one filter has a
                      value — keeps the panel clean when nothing is set. */}
                  {(dateFrom || dateTo || tagFilter) && (
                    <button
                      onClick={() => { setDateFrom(""); setDateTo(""); setTagFilter(""); }}
                      style={{
                        background: "none", border: `1px solid ${theme.border}`,
                        color: theme.textMuted, borderRadius: "8px",
            ...
            {/* Bulk action bar — enhanced with tag, re-type, export */}
            {bulkMode && bulkSelected.size > 0 && (
              <div style={{
                ...
                {/* Row 2: Bulk tag — type comma-separated tags then press Enter or click Tag.
                    Tags are ADDED to existing tags (union), not replaced. */}
                <div style={{
                  display: "flex", gap: "6px", alignItems: "center",
                  marginBottom: "8px",
                }}>
                  <input
                    type="text" value={bulkTagInput}
                ...
                {/* Row 3: Bulk re-type — expandable section.
                    Click "Change type" to reveal type buttons. Clicking a type
                    changes every selected item to that type and exits bulk mode. */}
                <div>
                  <button
                  ...
                  >
                    <span style={{
                      fontSize: "9px", transition: "transform 0.2s ease",
                  ...
                  {bulkRetypeOpen && (
                    <div style={{
                      ...
                      marginTop: "6px", animation: "softFadeIn 0.15s ease",
                    }}>
                      {Object.keys(typeLabels).map(type => {
                        const typeColor = TYPE_COLORS[type] || theme.textMuted;
                        return (
                          <button key={type}
                            onClick={() => { bulkRetype(bulkSelected, type); setBulkRetypeOpen(false); }}
                            style={{
                              background: `${typeColor}15`,
                              border: `1px solid ${typeColor}33`,
                              color: typeColor,
                              borderRadius: "14px", padding: "3px 10px", fontSize: "11px",
                              fontFamily: "'DM Sans', sans-serif", cursor: "pointer",
              ...
              {activeFiltered.length > 0 && (
                <div>
                  {groupItemsByDate(activeFiltered).map(group => (
                    <div key={group.label}>
                      {/* Date group header — label left-aligned with a line extending right.
                          Uses flexbox + a pseudo-line (1px div) to create the effect.
                          Extra top margin separates groups visually. */}
                      <div style={{
      ...
      {/* Undo delete toast — role="alert" makes screen readers announce this immediately */}
      {undoItem && (
